...
# Nexus Repository PyPI Setup Script
# This script sets up Nexus Repository for hosting PyPI packages
...
echo -e "${BLUE}🚀 Setting up Nexus Repository for PyPI packages...${NC}"
echo ""
...
check_nexus_running() {
    ...
    if curl -s -f "${NEXUS_URL}/service/rest/v1/status" > /dev/null 2>&1; then
        ...
        echo -e "${YELLOW}Please start Nexus Repository first:${NC}"
        echo -e "${BLUE}  docker run -d --name nexus -p 8081:8081 sonatype/nexus3:latest${NC}"
        return 1
    fi
}
...
# Function to create/ensure cleanup policy for dev packages older than 14 days
create_cleanup_policy() {
    echo -e "${BLUE}Creating/Updating cleanup policy: pypi-dev-14d${NC}"

    local policy_payload=$(cat <<EOF
{
  "name": "pypi-dev-14d",
  "format": "pypi",
  "notes": "Delete PyPI dev builds older than 14 days",
  "mode": "delete",
  "criteria": {
    "lastBlobUpdated": 14,
    "regex": ".*dev.*"
...
EOF
    ...
    # Try update first (PUT), then create (POST) if not exists
    if curl -s -f \
        -H "Content-Type: application/json" \
        -u "${NEXUS_ADMIN_USER}:${NEXUS_ADMIN_PASSWORD}" \
        -X PUT \
        -d "$policy_payload" \
        "${NEXUS_URL}/service/rest/beta/cleanup/policies/pypi-dev-14d" > /dev/null 2>&1; then
        echo -e "${GREEN}✅ Cleanup policy updated${NC}"
    elif curl -s -f \
        -H "Content-Type: application/json" \
        -u "${NEXUS_ADMIN_USER}:${NEXUS_ADMIN_PASSWORD}" \
        -X POST \
        -d "$policy_payload" \
        "${NEXUS_URL}/service/rest/beta/cleanup/policies" > /dev/null 2>&1; then
        echo -e "${GREEN}✅ Cleanup policy created${NC}"
    else
        echo -e "${YELLOW}⚠️  Failed to create or update cleanup policy (endpoint may differ by Nexus version)${NC}"
    fi
}

# Function to attach cleanup policy to hosted repository
attach_cleanup_policy_to_repo() {
    echo -e "${BLUE}Attaching cleanup policy to hosted repository: ${REPOSITORY_NAME}${NC}"
    local repo_update=$(cat <<EOF
{
  "name": "${REPOSITORY_NAME}",
  "online": true,
  "storage": {
...
EOF
    ...
    if curl -s -f \
        ...
        echo -e "${GREEN}✅ Cleanup policy attached to ${REPOSITORY_NAME}${NC}"
    else
        echo -e "${YELLOW}⚠️  Failed to attach cleanup policy to ${REPOSITORY_NAME}${NC}"
    fi
}
    ...
    local user_update=$(cat <<EOF
{
  ...
  "firstName": "PyPI",
  "lastName": "Publisher",
  "emailAddress": "pypi@plosolver.com",
...
display_instructions() {
    echo ""
    echo -e "${GREEN}🎉 Nexus Repository PyPI setup completed!${NC}"
    echo ""
...
# Main execution
main() {
    echo -e "${BLUE}🔧 Nexus Repository PyPI Setup${NC}"
    ...
    # Create cleanup policy and attach to hosted repository
    create_cleanup_policy
    attach_cleanup_policy_to_repo
